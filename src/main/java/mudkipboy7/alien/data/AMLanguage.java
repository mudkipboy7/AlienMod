...
import mudkipboy7.alien.AMConfig;
...
public class AMLanguage {
	...
	static class AmericanEnglishGenerator extends LanguageProvider {
		...
		protected void addTranslations() {
			...
			addBlockDesc(AMBlocks.TEST_BLOCK, "Used to test stuff during development.");
			addBlock(AMBlocks.AMMONIA_LIQUID_BLOCK, "Umifer");
			addBlockDesc(AMBlocks.AMMONIA_LIQUID_BLOCK, "A liquid soultion of ammonia and various other chemicals.");
			add(AMFluids.AMMONIA_LIQUID_TYPE.get().getDescriptionId(), "Umifer");
			addBlock(AMBlocks.ALIEN_AIR, "Alien Air");
			addBlockDesc(AMBlocks.ALIEN_AIR, "What! How are you holding air?");
			...
			addBlockDesc(AMBlocks.ALIEN_PORTAL, "A portal that leads to the alien dimension.");
			addBlock(AMBlocks.ALIEN_SAPLING, "Lignum Sapling");
			addBlock(AMBlocks.DEAD_PLANT, "Dead Plant");
			addBlockDesc(AMBlocks.DEAD_PLANT, "A plant that died from extreme conditions.");
			...
			addBlockDesc(AMBlocks.LAZER_CREATOR,
					"Creates a lazer that can be used to transport energy between machines.");
			addBlock(AMBlocks.LAZER_EXTENDER, "Lazer Extender");
			addBlockDesc(AMBlocks.LAZER_EXTENDER, "Extends a lazer to make it go a further distance.");
		...
		private void addBlockDesc(RegistryObject<? extends Block> block, String text) {
			this.add(getDescription(block.get()).getString(), text);
		}

		private void addItemDesc(RegistryObject<? extends Item> item, String text) {
			this.add(getDescription(item.get()).getString(), text);
		}
	...
	// Gives what the language name of a block's description should be.
	...
	// Gives what the language name of an item's description should be.
