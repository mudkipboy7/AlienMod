...
public class JovianDimCloudsRenderer {

	private static final ResourceLocation CLOUDS_LOCATION = AlienMod.location("textures/environment/jovian_clouds.png");
	...
	private static void drawCloudsAtHeight(int cloudHeight, ClientLevel level, int ticks, float partialTick,
			PoseStack poseStack, double camX, double camY, double camZ, Matrix4f projectionMatrix) {
		if (true) {
			...
			if (generateClouds) {
				...
				BufferBuilder.RenderedBuffer bufferbuilder$renderedbuffer = buildClouds(bufferbuilder, d2,
						d3, d4, vec3);
				cloudBuffer.bind();
	...
	private static BufferBuilder.RenderedBuffer buildClouds(BufferBuilder pBuilder, double pX, double pY, double pZ,
			Vec3 pCloudColor) {
