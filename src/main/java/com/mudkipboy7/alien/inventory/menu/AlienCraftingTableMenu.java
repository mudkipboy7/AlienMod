...
public class AlienCraftingTableMenu extends RecipeBookMenu<CraftingContainer> {
	...
	public AlienCraftingTableMenu(int pContainerId, Inventory pPlayerInventory) {
		this(pContainerId, pPlayerInventory, ContainerLevelAccess.NULL);
	}

	public AlienCraftingTableMenu(int pContainerId, Inventory pPlayerInventory, ContainerLevelAccess pAccess) {
	...
	protected static void slotChangedCraftingGrid(AbstractContainerMenu pMenu, Level pLevel, Player pPlayer,
		...
		if (!pLevel.isClientSide) {
			...
			Optional<RecipeHolder<CraftingRecipe>> optional = pLevel.getServer().getRecipeManager()
			...
			serverplayer.connection.send(
					new ClientboundContainerSetSlotPacket(pMenu.containerId, pMenu.incrementStateId(), 0, itemstack));
		}
	}
